...
FLEX_WEIGHT = 0.8          # tune: 0.2–1.0 works; keep << typical point gain
FLEX_RADIUS = 2            # how far to look for other claimable structsWORTH_IT_THRESHOLD = 0.5          # min delta to justify spending a meeple
SCARCITY_TABLE = [1.0, 0.9, 0.7, 0.4, 0.15, 0.0]  # idx = meeples_left (cap to len-1)
ENDGAME_TILE_CUTOFF = 15           # tiles remaining → “endgame”
ENDGAME_INSTANT_WEIGHT = 1.3       # boost for instant points late
CONTEST_BONUS = 1.0                # add to delta when you can tie/steal majority
RISK_PENALTY_PER_OPEN_END = 0.15   # subtract for big unfinished cities
RANDOM_JITTER = 0.0001             # tiny epsilon to break deterministic ties

def local_structure_density(game, x, y, radius=FLEX_RADIUS) -> float:
    ...
    for dx in range(x-radius, x+radius+1):
        for dy in range(y-radius, y+radius+1):
            ...
            if 0 <= dx < len(grid[0]) and 0 <= dy < len(grid):
                total += 1
                t = grid[dy][dx]
                if t is not None and any(v is None for v in t.internal_claims.values()) is False:
                    # tile exists AND its claimable spots are mostly taken -> “crowded”
                    hits += 1
...
class TileStructData:
    ...
    def compute_score_gain_with_meeple(self, game) -> float:
        ...
        nearby_pts = self.get_nearby_monastary_score(game)

        if self.struct_type == StructureType.MONASTARY:
            score = self.get_monastary_score(game) + nearby_pts
            # flex bonus still helps diversify
            density = local_structure_density(game, self.x, self.y)    
            return score + FLEX_WEIGHT * (1 - density)
        if self.is_completed:
            score = self.compute_completed_score(game) + nearby_pts + MEEPLE_FREE_SCORE_MODIFIER
            density = local_structure_density(game, self.x, self.y)
            return score + FLEX_WEIGHT * (1 - density)
        
        complete_probability = 1 / (1 + self.num_open_ends)
        score = (
                self.compute_incomplete_score(game) * (1 - complete_probability)+ 
                self.compute_completed_score(game) * complete_probability +
                nearby_pts
        )  

        density = local_structure_density(game, self.x, self.y)
        flex_bonus = FLEX_WEIGHT * (1 - density)

        return score + flex_bonus
    ...
    def completion_prob(self, open_ends) -> float:
        ...
        return 1 / (1 + (factor * open_ends) ** 2.5)


    def compute_score_gain_without_meeple(self, game) -> float:
        nearby_pts = self.get_nearby_monastary_score(game)

        # Flexibility bonus still matters when we *don’t* place a meeple
        density = local_structure_density(game, self.x, self.y)
        flex_bonus = FLEX_WEIGHT * (1 - density)

        # Monastery: you get nothing for skipping, just nearby monastery points
        if self.struct_type == StructureType.MONASTARY:
            return nearby_pts + flex_bonus

        # Not dominant → effectively zero from this feature; still keep nearby + flex
        if self.my_meeples_active * 2 <= self.meeples_active:
            return nearby_pts + flex_bonus

        # If it’s already completed, you’re cashing out now without placing
        if self.is_completed:
            score = (self.compute_completed_score(game) +
                     self.my_meeples_active * MEEPLE_FREE_SCORE_MODIFIER +
                     nearby_pts)
            return score + flex_bonus

        # EV delta of extending the feature by one tile without committing a meeple
        ...
        # include current incomplete value (keeping dominance) + nearby + flex
        return gain + incomplete + nearby_pts + flex_bonus
    ...
    def compute_completed_score(self, _):
        ...
        match self.struct_type:
            case StructureType.CITY:
                ...
                prev_structs = self.num_structs - 1
                prev_emblems = self.num_emblems

                if TileModifier.EMBLEM in self.tile.modifiers:
                    prev_emblems -= 1

                old_score = prev_structs + prev_emblems
                new_score = 2 * (self.num_structs + self.num_emblems)
...
class TileData:
    ...
    def compute_highest_score_with_immediate_meeple_return(self, game):
        ...
        total_non_meeple_scores = self.compute_effective_score_without_meeple(game)
...
def analyse_board (game: Game, placement_list: list[tuple[int,int,int,Tile, int,dict[StructureType,list[Tile]]]]):
    ...
    best_immediate_return_option = 0

    best_meeple_edge = None
    best_meeple_edge_immediate = None

    data = []


    for i, option in enumerate(placement_list):
        x, y, r, tile, tile_idx, _ = option
        ...
        score_immediate_return, meeple_edge_immediate = tile_data.compute_highest_score_with_immediate_meeple_return(game)


        # 1. Save objectively best decision where meeple is used (highest EV)
        # 2. Save best decision where a meeple is return immediately - i.e. can be used even if only 1 meeple left.
        # 3. Save best option where no meeple is used, as using a meeple doesnt make much of a difference.
